...
$('#form01').submit(function (event) {
   event.preventDefault()
    ...
    if ($('#pseudo').val().length < 3 ||$('#comment').val().length < 8){
        alert("Champs incomplet")
          ...
          var newcomment =` <h4>%pseudo%</h4> 
                            <p>%comment%</p>
                          `
            newcomment = newcomment.replace(/%pseudo%/g,$('#pseudo').val())
          
            newcomment = newcomment.replace(/%comment%/g,$('#comment').val())
          
           $('#defil').append(newcomment)
